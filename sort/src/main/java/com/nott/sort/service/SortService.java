...
public class SortService {
    ...
    public String sort(Map<String, Object> map) {
        ...
        if (StringUtils.isEmpty(type)) {
            return "请选择一种排序方式：[bubble, selection, insertion, shell, merge, quick, heap, counting, bucket, radix]";
        }
        int arr[] = {9, 2, 0, 3, 7, 4, 6, 5};
        Sort sort = sortFactory.getService(type);
        if (sort == null) {
            return "请输入正确的排序方式：[bubble, selection, insertion, shell, merge, quick, heap, counting, bucket, radix]";
        }
        try {
            sort.sort(arr);
        } catch (Exception e) {
